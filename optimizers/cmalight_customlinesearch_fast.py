...
# def get_w(model):
#     return torch.cat([p.ravel().detach() for p in model.parameters()])
...
class CMA_LV2Fast(torch.optim.Optimizer):
    ...
    def EDFL(self,
        ...
        alpha = zeta * eta
        nfev = 0
        sample_model = copy.deepcopy(mod.to(device)) # Trasferisci modello su GPU
        real_loss = closure(dl_train,sample_model,criterion,device)

        if norm_d_k is None: norm_d_k = torch.linalg.norm(d_k)  # Calcolo della norma una volta sola
        ...
        while cur_loss <= min(f_j, real_loss - gamma * alpha * norm_d_k ** 2) and idx <= self.defaults.get('max_it_EDFL'):
            ...
            with torch.no_grad():
                idxx = 0
                for param in sample_model.parameters():
                    param.data.copy_(w_prova[idxx:idxx + param.numel()].view(param.shape))  # Modifica direttamente i parametri
                    idxx += param.numel()
            cur_loss = closure(dl_train, sample_model, criterion, device)
            nfev += 1
            idx += 1
